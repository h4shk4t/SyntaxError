...
def chord(scale,indR,midR,ringR,pinkR):
    if (indR == 2 and midR == 2 and ringR ==2 and pinkR == 2):
        ...
        return "Am"
    elif ("Em" in scale and (((indR == 2 and midR == 2 and ringR ==0) or (midR==2 and ringR == 2 and indR == 0))) and pinkR == 0):
        return "Em"
...
def play(chord,strum):
    print(chord)
    if (chord == "C"):
        playsound("C_64kb.mp3")
    elif (chord == "D"):
        playsound("D_64kb.mp3")
    elif (chord == "Dm"):
        playsound("Dm_64kb.mp3")
    elif(chord == "E"):
        playsound("E_64kb.mp3")
    else:
...
@app.route("/video")
def video():
    #Mimetype list ka toh alag hai
    return Response(gen_run(),mimetype='multipart/x-mixed-replace; boundary=frame')
